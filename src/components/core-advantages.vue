...
<template>
  <div class="flex justify-center items-center w-full my-8 sm:my-4" style="z-index: 1">
    <div
    ...
    <div
      ...
      <p class="leading-normal font-bold text-2xl font-format text-mainG mb-4 sm:text-xs">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getImage = (): void => {
        list.map((val: number) => {
          const obj: IImge = { img: '' }
          obj.img = imgImport[`../assets/img/advant-${props.type}` + val + '.png'].default
          imgList.push(obj)
        })
      }
      const img = computed(() => {
