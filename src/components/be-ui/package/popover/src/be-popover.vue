  ...
  export default defineComponent({
    ...
    setup(props, ctx) {
      ...
      const changeDisplay = (showParams: boolean | string): void => {
        ...
        if (isShow) delay = props.delay
        else delay = props.duration

        setTimeout(() => {
          // 关闭时如果鼠标移入了popover的dom，则不关闭
          if (isEnterPopover.value && !isShow && props.trigger === 'hover') return
      ...
      // 位置样式
      const stylePopover: TPopoverStyle = reactive({
        left: '0px',
        top: '0px',
        zIndex: '2000',
      })
       ...
       */
      const computePosition = (placement: string | undefined, type = ''): void => {
        if (type === 'update') {
          popperJS.value?.update()
          return
        }
        // 使用popover.js 对popover进行定位
        if (popperJS.value && popperJS.value?.destroy) popperJS.value?.destroy()
        ...
        if (props.x && props.y) {
          // 传入了坐标就 创建虚拟trigger
          const VNodeTrigger: VirtualElement = {
      ...
      let observer =
        ...
        new MutationObserver(() => {
          if (props.forceUpdate) popperJS.value?.update()
        })
      /**
      ...
      const matchDom = (root: any): any => {
        for (let i = 0; i < root.childNodes.length; i++) {
          const node: HTMLElement = root.childNodes[i]
          if (node.nodeType !== 3 && node.nodeType !== 8 && node.nodeName !== 'SCRIPT') {
            const triggerWidth = Number(window.getComputedStyle(node).width.split('px')[0])
            const triggerHeight: number = node.getBoundingClientRect().height
            if (triggerWidth !== 0 || triggerHeight !== 0) return node
            else return matchDom(node)
          }
        }
      }
      /**
      ...
      const addEvent = (trigger?: HTMLElement): void => {
        if (ctx.slots.trigger) {
          const triggerElm: HTMLElement | string | undefined = trigger || props.triggerElm
          if (triggerElm) {
      ...
      const outsideDisabled = ref<boolean>(false)
      onMounted((): void => {
        // 根据触发类型 设置是否禁用 click-outside,只有click触发类型时才使用
        if (props.trigger === 'hover' || props.trigger === 'manual') outsideDisabled.value = true

        // 禁用就不绑定触发事件了
        if (props.disabled) return

        if (props.width) stylePopover.width = `${props.width}px`
      ...
      const handlePopoverDomEnter = (): void => {
        if (props.trigger === 'hover') isEnterPopover.value = true
      }
    ...
    <div
      ...
      <transition name="be-fade-in-linear">
        <div
          ...
          <div v-if="raw" :id="`be_popover_arrow${uid}`" class="be-popover--arrow" />
        </div>
      </transition>
    </div>
  </teleport>
