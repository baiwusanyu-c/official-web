/* * be-input.vue * @deprecated 带输入建议远程搜索的输入框 * @author czh * @create (czh 2021/5/31) *
@update (czh 2021/6/2) */
<template>
  <div
    :id="`be_input_${uid}`"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, ctx) {
      ...
      const inputType = ref<string>(props.type as string)
      const isPassWordInitVal: boolean = inputType.value === 'password' ? true : false
      const isPassWord = ref<boolean>(isPassWordInitVal)
      /**
       * 密码显示按钮方法
       */
      const handlePassword = (): void => {
        ...
        if (!isPassWord.value) {
          ...
          inputType.value = props.type as string
        }
      }
