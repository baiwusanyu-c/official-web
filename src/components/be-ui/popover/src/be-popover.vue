...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const changeDisplay = (showParams: boolean | string): void => {
      let delay = 0
      const isShow =
        showParams === 'manual' ? !show.value : (showParams as boolean)
      if (isShow) {
        delay = props.delay
      } else {
    ...
    const computePosition = (placement: string, type = ''): void => {
      ...
      if (props.x && props.y) {
        ...
        VNodeTrigger.getBoundingClientRect = generateGetBoundingClientRect(
          props.x,
          props.y
        )
        popperJS.update()
      ...
      for (let i = 0; i < root.childNodes.length; i++) {
        ...
        if (
          ...
          let triggerWidth = Number(
            window.getComputedStyle(node).width.split('px')[0]
          )
    ...
    const removeEvt = (): void => {
      if (triggerDom) {
        ...
        triggerDom.removeEventListener(
          'mouseenter',
          evtList['mouseenter'],
          false
        )
