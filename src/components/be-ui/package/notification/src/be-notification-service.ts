...
import { jsonClone } from '../../../../src/utils/common'
import beNotifyComponents from './be-notification'
import type { DefineComponent } from 'vue'
import type { INotfiyInst, INotifyOption, ItInstanceMap } from './be-notification-type'
...
const closeNotify = function (instance: DefineComponent, isAll = false): void {
  ...
  for (let i = index; i < len - 1; i++) {
    instancesList[i].instance.el.style[direction] = `${
      parseInt(instancesList[i].instance.el.style[direction], 10) - removedHeight - 35
    }px`
  }
...
const setCloseAnimate = (compInstance: HTMLElement): void => {
  ...
  if (!className.includes('be-message')) return

  if (className.includes('-in')) className = className.replace(/-in/, '-out')
  else className = `${className} be-message-animation-top-center-out`
...
const componentRender = (
  ...
  if (!instanceInner) {
    ...
    instanceInner = createVNode(beNotifyComponents, {
      option,
      compType: option.compType,
    })
    ...
    if (bodyElement && bodyElement.append) bodyElement.append(instanceInner.el)
    else bodyElement && bodyElement.appendChild(instanceInner.el)
  } else {
...
const createNotify = function (options: INotifyOption): INotfiyInst {
  ...
  if (option.placement === undefined) instanceArr = instanceMap.topRight as Array<any>
  else instanceArr = Object(instanceMap)[option.placement]
